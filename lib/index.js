...
var icons = process.argv.slice(2);
if (icons.length === 0) {
	icons = glob.sync(path.join(__dirname, '../ionicons/src/*.svg')).map(function(filename) {
...
var bar = new ProgressBar('Running Tests: [:bar] :percent ', {
...
(function runTest(i, j) {
	if (j < types.length) {
		if (i < icons.length) {
			var url = ['http://localhost:8080/', types[j], '/', icons[i], '.html'].join('');
